...
namespace WeWereBound
{
    public class Commands
    {
        ...
        public void Log(object obj, Color color)
        {
            ...
            int maxCommands = (GameEngine.Instance.Window.ClientBounds.Height - 100) / 30;
        ...
        private void LogStackTrace(string stackTrace)
        {
            foreach (var call in stackTrace.Split('\n'))
            {
                ...
                GameEngine.Commands.Log(log, Color.White);
            }
        }
        ...
        private static void Count(int tagIndex = -1)
        {
            ...
            if (tagIndex < 0)
                GameEngine.Commands.Log(GameEngine.Scene.Entities.Count.ToString());
            else
                GameEngine.Commands.Log(GameEngine.Scene.TagLists[tagIndex].Count.ToString());
        }
        ...
        private static void Tracker(string mode)
        {
            ...
            switch (mode)
            {
                default:
                    ...
                    GameEngine.Commands.Log("-- Components --");
                    GameEngine.Scene.Tracker.LogComponents();
                    GameEngine.Commands.Log("-- Collidable Components --");
                    GameEngine.Scene.Tracker.LogCollidableComponents();
                    break;
        ...
        private static void Help(string command)
        {
            if (GameEngine.Commands.sorted.Contains(command))
            {
                ...
                GameEngine.Commands.Log(str.ToString());
            ...
            else
            {
                ...
                GameEngine.Commands.Log(str.ToString());
                GameEngine.Commands.Log("Type 'help command' for more info on that command!");
            }
        }
#endif
        #endregion
